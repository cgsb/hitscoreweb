...
let all_pgm_runs ~configuration =
  ...
  with_database ~configuration (fun ~dbh ->
      ...
      layout#pgm_run#all >>= fun all_pgm_runs ->
      layout#pgm_pool#all >>= fun all_pgm_pools ->
      layout#invoicing#all >>= fun all_invoicings ->
      layout#person#all >>= fun all_persons ->
      layout#pgm_input_library#all >>= fun all_pgm_input_libs ->
      while_sequential all_pgm_input_libs (fun pil ->
          pil#library#get
          >>= fun sl ->
          return (pil, sl)
        )
      >>= fun all_pgm_stock_libs ->
