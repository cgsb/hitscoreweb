...
(* CHECK *)
let scope = `Session (Eliom_common.create_scope_name "__hswe_eliom_openid")
let group_name = "__hswe_eliom_openid_group"
...
let init ~path ~f ~realm = 
  let module K = Make (struct let path = path let f = f let realm = realm end) in
  (check K.authenticate : check_fun)
